...
class Main {
  ...
  public static void customCode() {
    ...
    //ArrayList<Rule> rules = MyGrammar.rulesList();

    ArrayList<Word> BFwordsList = new ArrayList<Word>();
    BFwordsList.add(new Word("x+x"));
    BFwordsList.add(new Word("01"));
    BFwordsList.add(new Word("1+0"));
    BFwordsList.add(new Word("*-*"));

    System.out.print("Brute Force method: \n");

    for(Word item : BFwordsList){
      System.out.println("\nIs " + item + " in the language:");
      System.out.println(parser.isInLanguage(cfg, item));
      System.out.println("\nParser tree for " + item + ": \n");
      System.out.println(parser.generateParseTree(cfg, item));
    }

    ArrayList<Word> CYKwordsList = new ArrayList<Word>();
    CYKwordsList.add(new Word("1*-0*-1+0*-1"));
    CYKwordsList.add(new Word("0+-1*-0+1*1"));
    CYKwordsList.add(new Word("1*0+--1+0*1"));
    CYKwordsList.add(new Word("1*0+0-1+1*0+-0"));

    System.out.print("\nCYK algorithm method: \n");
    
    for(Word item : CYKwordsList){
      System.out.println("\nIs " + item + " in the language:");
      System.out.println(cykparser.isInLanguage(cfg, item));
    }
  }
   ...
   * line.
   * 
   * There is nothing stopping you from editing it if you want, but make sure you
  ...
  public static void main(String[] args) {
    if (!SKIP_TO_TESTS) {
      System.out
          .println("|------- Welcome to the demo script. I recommend turning on word wrap in your console. -------|\n"
  ...
  public static void explainAssignment() {
    ...
    System.out.println(
        "The next and main part of the assignment is to write a parser, then apply it to the grammar you created.\n");
  ...
  private static void demoCode() {
    ...
    System.out.println("And there is a helper method to create an array of subscripted variables:\n"
        + "\te.g. Variable.subscriptedVariables('S', 4); \t-- this produces: "
        + Arrays.toString(Variable.subscriptedVariables('S', 4)));
    ...
    System.out.println(
        "\t     new Word(\"A0\"); \t\t-- this produces: " + new Word("A0") + " (variable followed by terminal)");
    System.out.println();

    System.out.println("The other way to construct a Word is with multiple symbols:");
    System.out.println("\te.g. new Word(new Variable(\"A0\"), new Terminal('0')); \t-- this produces: "
        + new Word(new Variable("A0"), new Terminal('0')));
    System.out.println();
